...
export default {
	...
	methods: {
		...mapActions('calendarSettingsStore', ['fetchMyCalendarData']),
		...mapMutations('utils', ['CURRENT_USER_DATA']),
		fetchScheduleData () {
			const queryString = `?username=${this.currentUserName}`
			this.fetchMyCalendarData({ queryString, success: this.checkCallBack })
		},
		refreshCalendar () {
		...
		checkCallBack () {
            ...
            debugger
			for (const item of this.getMyCalendarData) {
				const obj = {
	...
	computed: {
		...mapGetters('calendarSettingsStore', ['getMyCalendarData'])
	},
